...
void encounte(Player& player, Enemy& enemy) {
    ...
    while (player.isAlive() && enemy.isAlive()) {
        ...
        else if ("2" == action) { // heal
            player.health += player.healing;
            if (player.healing >= 40)
            {
                player.healing = 39;
            }
        ...
        while (true) { // Начало игры
            ...
            if (P_X != prevX || P_Y != prevY) { // если после всех изменений позиция изменилось то
                ...
                if (result == true) { // если скажет да то
                    ...
                    for (int i = 1; i < rand() % 7 + 1; i++) {
                        int y = rand() % MAX_Y_VIEV;
                        int x = rand() % MAX_X_VIEV;

                        while (BattleField[y][x] == '#') {
                            y = rand() % MAX_Y_VIEV;
                            x = rand() % MAX_X_VIEV;
                        }
                        BattleField[y][x] = '!';
